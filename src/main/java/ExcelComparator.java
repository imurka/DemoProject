...
//    }
     ...
     */
    public static List<String> compare(Workbook wb1, Workbook wb2) throws IOException {
        ...
        excelComparator.compareSheetData( wb2, loc1, loc2);
     ...
     */
    private void compareDataInAllSheets(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        for (int i = 0; i < loc1.workbook.getNumberOfSheets(); i++) {
            ...
            compareDataInSheet(wb2, loc1, loc2);
        }
    }

    private void compareDataInSheet(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        for (int j = 0; j < loc1.sheet.getPhysicalNumberOfRows(); j++) {
            ...
            compareDataInRow(wb2,loc1, loc2);
        }
    }

    private void compareDataInRow(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        for (int k = 0; k < loc1.row.getLastCellNum(); k++) {
            ...
            compareDataInCell(wb2, loc1, loc2);
        }
    }

    private void compareDataInCell(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        if (isCellTypeMatches(loc1, loc2)) {
            ...
            switch(loc1cellType) {
                ...
                case STRING:
                case ERROR:
                    isCellContentMatches(wb2, loc1,loc2);
                    break;
                case BOOLEAN:
     ...
     */
    private void compareNumberOfColumnsInSheets(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        for (int i = 0; i < loc1.workbook.getNumberOfSheets(); i++) {
            if (loc2.workbook.getNumberOfSheets() <= i) return;
     ...
     * Compare sheet data.
     */
    private void compareSheetData(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
        compareNumberOfRowsInSheets(loc1, loc2);
        compareNumberOfColumnsInSheets(wb2, loc1, loc2);
        compareDataInAllSheets(wb2,loc1, loc2);
     ...
     */
    private void isCellContentMatches(Workbook wb2, Locator loc1, Locator loc2) throws IOException {
    ...
    private void highlightDifference(Workbook wb2, Locator loc2) throws IOException {
...
//        style.setFillBackgroundColor(IndexedColors.BLUE.getIndex());
//        loc2.cell.setCellStyle(style);

        System.out.println("highlightDifference");

        loc2.cell.getCellStyle().setFillPattern(FillPatternType.SOLID_FOREGROUND);
        loc2.cell.getCellStyle().setFillForegroundColor(IndexedColors.YELLOW.getIndex());





        //setFillBackgroundColor(IndexedColors.PINK.getIndex());
